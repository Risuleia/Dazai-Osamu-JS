const { slap } = require('../../Utilities/roleplay.js')

module.exports = {
	name: 'slap',
	aliases: [],
	usage: '<member>',
	description: "Sends a slapping gif.",
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to slap.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} slaps${target}`,
				icon_url: message.member.displayAvatarURL({ dynamic: true }) 
			},
			image: { url: await slap() }
		}
