const { kill } = require('../../Utilities/roleplay.js')

module.exports = {
	name: 'kill',
	aliases: [],
	usage: '<member>',
	description: "Sends a killing gif.",
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to kill.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} kills${target}`,
				icon_url: message.member.displayAvatarURL({ dynamic: true }) 
			},
			image: { url: await kill() }
		}
