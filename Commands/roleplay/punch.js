const { punch } = require('../../Utilities/roleplay.js')

module.exports = {
	name: 'punch',
	aliases: [],
	usage: '<member>',
	description: "Sends a punching gif.",
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to punch.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} punches${target}`,
				icon_url: message.member.displayAvatarURL({ dynamic: true }) 
			},
			image: { url: await punch() }
		}
