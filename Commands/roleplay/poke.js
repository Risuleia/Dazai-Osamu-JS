const { poke } = require('../../Utilities/roleplay.js')

module.exports = {
	name: 'poke',
	aliases: [],
	usage: '<member>',
	description: "Sends a poking gif.",
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to poke.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} pokes${target}`,
				icon_url: message.member.displayAvatarURL({ dynamic: true }) 
			},
			image: { url: await poke() }
		}
