const { kiss } = require('../../Utilities/roleplay.js')

module.exports = {
	name: 'kiss',
	aliases: [],
	usage: '<member>',
	description: "Sends a kissing gif.",
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to kiss.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} kisses${target}~ aww`,
				icon_url: message.member.displayAvatarURL({ dynamic: true }) 
			},
			image: { url: await kiss() }
		}
