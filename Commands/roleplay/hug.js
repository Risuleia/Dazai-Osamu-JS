const { hug } = require('../../Utilities/roleplay.js')
...
module.exports = {
	...
	execute: async (client, message, args, db) => {
		...
		let target;
		if (user == false) return message.reply("You need to specify someone to hug.")
		...
		const embed = {
			color: 0xB2A7FF,
			author: {
				name: `${message.member.displayName} hugs${target}~ adorable`,
